...
The size of the data set is `r nrow(plot_df)` rows and `r ncol(plot_df)` columns. The mean is `r mean(x)`. `r sum(`x + y > 0`, na.rm=TRUE)`   
...
ggplot(plot_df, aes(x = x, y = y)) + geom_point(aes(color = as.factor(as.logical(x + y > 0))))
